...
fileID = fopen('CH1_ÁÖ¸Ô1.txt','r');
formatSpec = '%f';
sizeA = [1 Inf];
A = fscanf(fileID,formatSpec,sizeA);
fclose(fileID);
fileID = fopen('CH2_ÁÖ¸Ô1.txt','r');
formatSpec = '%f';
sizeB = [1 Inf];
B = fscanf(fileID,formatSpec,sizeB);
fclose(fileID);
fileID = fopen('CH3_ÁÖ¸Ô1.txt','r');
formatSpec = '%f';
sizeC = [1 Inf];
...
Amp2 = B(locs);
Amp3 = C(locs);
fileID = fopen('Ch1_¹¬1.txt','w');
%fprintf(fileID,'%12s %12s %12s\n','Ch1','Ch2','Ch3');
fprintf(fileID,'%f\n',Amp1);
fclose(fileID);
fileID = fopen('Ch2_¹¬1.txt','w');
fprintf(fileID,'%f\n',Amp2);
fclose(fileID);
fileID = fopen('Ch3_¹¬1.txt','w');
fprintf(fileID,'%f\n',Amp3);
fclose(fileID);
%% Plot rectified EMG
subplot(2,2,1)
